...
function Menu:draw()
	...
	moon.graphics.draw(graphics["logo"], 400, 125, 0, 1, 1, 100, 75)
...
function Instructions:draw()
	...
	moon.graphics.set_color(table.unpack(color["text"]))
	-- moon.graphics.set_font(font["small"])
	moon.graphics.printf("The point of this game is to fill out a standard, randomly generated, nonogram by using the mouse. The left mouse button fills in (or \"un-fills\") an area whilst the right mouse button is used to set hints where you think an area shouldn't be filled.\nUse the escape key to pause the game.\n\nGood luck.", 100, 250, 600, "center")
...
function Options:draw()
	...
	moon.graphics.set_color(table.unpack(color["text"]))
	-- moon.graphics.set_font(font["large"])
	moon.graphics.print("Audio:", 250, 270)
	moon.graphics.print("Level:", 250, 345)
	
	moon.graphics.set_color(table.unpack(color["main"]))
	moon.graphics.set_line_width(4)
	
	if audio then
		moon.graphics.line(400,305,450,305)
	else
		moon.graphics.line(525,305,575,305)
	end
	
	moon.graphics.line(360+((size-5)*50),380,390+((size-5)*50),380)
...
function Game:draw()
	...
	local gs = size*50
	local gx = (moon.graphics.get_width() - gs) / 2
	local gy = (moon.graphics.get_height() - gs) / 2 + (size / 2 * 10)	
	local offset = 0
	...
	for x=1,size do
		for y=1,size do
			if self.grid[x][y] == 1 then
				moon.graphics.draw(graphics["set"], gx+(x*50)-25, gy+(y*50)-25, 0, 1, 1, 25, 25)
			elseif self.grid[x][y] == 2 then
				moon.graphics.draw(graphics["notset"], gx+(x*50)-25, gy+(y*50)-25, 0, 1, 1, 25, 25)
			end
		end
	end
	...
	moon.graphics.rectangle("line",gx,gy,gs,gs) -- surrounding rectangle
	moon.graphics.set_line_width(1)
	for i=1,size do
		offset = offset + (gs/size)
		moon.graphics.line(gx+offset, gy, gx+offset, gy+gs) -- vertical lines
		moon.graphics.line(gx, gy+offset, gx+gs, gy+offset) -- horizontal lines
	end
	
	-- Text
	moon.graphics.set_color(table.unpack(color["text"]))
	-- moon.graphics.set_font(font["default"])
	for i=1,size do
		-- moon.graphics.printf(self.horizontal[i],
		-- 			gx+(50*i)-50,
		-- 			gy-((self.horizontal[i]:len()/2) * font["default"]:get_height() * font["default"]:getLineHeight()),
		-- 			50, "center")
		moon.graphics.printf(self.horizontal[i],
					gx+(50*i)-50,
					gy,
					50, "center")		
		moon.graphics.printf(self.vertical[i], 0, gy+(50*i)-36, gx, "right")
	end
	
	-- Time (removed)
	--moon.graphics.set_color(color["text"])
	--moon.graphics.set_font(font["default"])
	--moon.graphics.draw(string.format("%.2fs", self.time), 700, 40)
	
	if self.win ~= -999 then
		-- You won!
		if self.win > 0 then
			moon.graphics.set_color(255,255,255,235-(235*(self.win/0.5)))
			moon.graphics.rectangle("fill",0,0,moon.graphics.get_width(),moon.graphics.get_height())
		else
			moon.graphics.set_color(table.unpack(color["overlay"]))
			moon.graphics.rectangle("fill",0,0,moon.graphics.get_width(),moon.graphics.get_height())
			moon.graphics.set_color(table.unpack(color["main"]))
			-- moon.graphics.set_font(font["huge"])
			moon.graphics.printf("CONGRATULATIONS", 0, 150, moon.graphics.get_width(), "center")
			moon.graphics.set_color(table.unpack(color["text"]))
			-- moon.graphics.set_font(font["default"])
			moon.graphics.printf("You completed a level " .. size .. " puzzle in: \n" .. string.format("%.2f", self.time) .. " seconds", 0, 200+64, moon.graphics.get_width(), "center")
			-- Buttons
	...
	elseif self.pause then
		...
		moon.graphics.set_color(table.unpack(color["main"]))
		-- moon.graphics.set_font(font["huge"])
		moon.graphics.printf("PAUSED", 0, 150, moon.graphics.get_width(), "center")
		moon.graphics.set_color(table.unpack(color["text"]))
		-- moon.graphics.set_font(font["default"])
		-- Buttons
	...
	elseif self.pause then
		...
		if x > 0
			...
			if button == "l" then
				...
				if audio then moon.audio.play(sound["shush"]) end
			...
			if self:testSolution() then
				if audio then moon.audio.play(sound["pling"]) end
				self.win = 0.5
			end
		end
