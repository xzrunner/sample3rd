...
function loveframes.util.GetDirectoryContents(dir, t)
	...
	files = moon.filesystem.get_directory_items(dir)
	
	for k, v in ipairs(files) do
		local isdir = moon.filesystem.is_directory(dir.. "/" ..v)
		if isdir == true then
...
function loveframes.util.IsCtrlDown()
	if moon._os == "OS X" then
		return moon.keyboard.isDown("lmeta") or moon.keyboard.isDown("rmeta") or
			moon.keyboard.isDown("lgui") or moon.keyboard.isDown("rgui")
	end
	return moon.keyboard.isDown("lctrl") or moon.keyboard.isDown("rctrl")
end