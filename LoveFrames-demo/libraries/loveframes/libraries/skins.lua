...
function loveframes.skins.Register(skin)
	-- todo zz
	-- print("loveframes.skins.Register", skin.name)
	...
	local dircheck = moon.filesystem.is_directory(dir)
	...
	local imagedircheck = moon.filesystem.is_directory(imagedir)
	...
	if indeximages then
		...
		for k, v in ipairs(images) do
			...
			if extension ~= "db" then
				loveframes.skins.available[name].images[key] = moon.scene_graph.new_scene_node(fullpath)
			end
		end
	end
...
function loveframes.skins.ReloadImages(name)
	...
	if skin and indeximages then
		...
		local dircheck = moon.filesystem.is_directory(imagedir)
		if dircheck then
			local images = loveframes.util.GetDirectoryContents(imagedir)
			for k, v in ipairs(images) do
				local filename = v.name
				local extension = v.extension
				local fullpath = v.fullpath
				loveframes.skins.available[name].images[filename .. "." .. extension] = moon.scene_graph.new_scene_node(fullpath)
			end
		end
	end
