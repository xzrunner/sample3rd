...
-- loveframes.basicfont = moon.graphics.newFont(12)
-- loveframes.basicfontsmall = moon.graphics.newFont(10)
loveframes.objects = {}
...
function loveframes.update(dt)
	...
	if version == "0.9.0" then
		...
		local arrow = moon.mouse.getSystemCursor("arrow")
		local curcursor = moon.mouse.getCursor()
		if hoverobject then
			local ibeam = moon.mouse.getSystemCursor("ibeam")
			...
			elseif hoverobject.type == "frame" then
				if not hoverobject.dragging and hoverobject.canresize then
					if loveframes.util.BoundingBox(hoverobject.x, mx, hoverobject.y, my, 5, 1, 5, 1) then
						local sizenwse = moon.mouse.getSystemCursor("sizenwse")
						if curcursor ~= sizenwse then
							moon.mouse.setCursor(sizenwse)
						end
					...
					elseif loveframes.util.BoundingBox(hoverobject.x + hoverobject.width - 5, mx, hoverobject.y, my, 5, 1, 5, 1) then
						local sizenesw = moon.mouse.getSystemCursor("sizenesw")
						if curcursor ~= sizenesw then
							moon.mouse.setCursor(sizenesw)
						end
					...
					elseif loveframes.util.BoundingBox(hoverobject.x + 5, mx, hoverobject.y, my, hoverobject.width - 10, 1, 2, 1) then
						local sizens = moon.mouse.getSystemCursor("sizens")
						if curcursor ~= sizens then
							moon.mouse.setCursor(sizens)
						end
					...
					elseif loveframes.util.BoundingBox(hoverobject.x, mx, hoverobject.y + 5, my, 2, 1, hoverobject.height - 10, 1) then
						local sizewe = moon.mouse.getSystemCursor("sizewe")
						if curcursor ~= sizewe then
							moon.mouse.setCursor(sizewe)
						end
...
function loveframes.draw()
	...
	local base = loveframes.base
	local r, g, b, a = moon.graphics.get_color()
	local font = moon.graphics.get_font()
	...
	moon.graphics.set_color(r, g, b, a)
