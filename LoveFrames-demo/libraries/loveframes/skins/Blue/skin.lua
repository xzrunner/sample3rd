...
-- local smallfont = moon.graphics.newFont(10)
-- local imagebuttonfont = moon.graphics.newFont(15)
...
function skin.OutlinedRectangle(x, y, width, height, ovt, ovb, ovl, ovr)
	...
	-- top
	if not ovt then
		moon.graphics.rectangle("fill", x, y, width, 1)
	end
	
	-- bottom
	if not ovb then
		moon.graphics.rectangle("fill", x, y + height - 1, width, 1)
	end
	
	-- left
	if not ovl then
		moon.graphics.rectangle("fill", x, y, 1, height)
	end
	
	-- right
	if not ovr then
		moon.graphics.rectangle("fill", x + width - 1, y, 1, height)
	end
...
function skin.DrawFrame(object)
	...
	local topbarimage_width = topbarimage:get_width()
	local topbarimage_height = topbarimage:get_height()
	...
	local bodyimage_width = bodyimage:get_width()
	local bodyimage_height = bodyimage:get_height()
	...
	moon.graphics.draw(bodyimage, x, y, 0, bodyimage_scalex, bodyimage_scaley)
	
	-- frame top bar
	moon.graphics.set_color(255, 255, 255, 255)
	moon.graphics.draw(topbarimage, x, y, 0, topbarimage_scalex, topbarimage_scaley)
	
	-- frame name section
	moon.graphics.set_font(font)
	
	if icon then
		local iconwidth = icon:get_width()
		local iconheight = icon:get_height()
		-- todo zz
		-- icon:setFilter("nearest", "nearest")
		moon.graphics.set_color(255, 255, 255, 255)
		moon.graphics.draw(icon, x + 5, y + 5)
		moon.graphics.set_color(namecolor)
		moon.graphics.print(name, x + iconwidth + 10, y + 5)
	else
		moon.graphics.set_color(namecolor)
		moon.graphics.print(name, x + 5, y + 5)
	end
	
	-- frame border
	moon.graphics.set_color(bordercolor)
	skin.OutlinedRectangle(x, y, width, height)
	
	moon.graphics.set_color(255, 255, 255, 70)
	skin.OutlinedRectangle(x + 1, y + 1, width - 2, 24)
	
	moon.graphics.set_color(220, 220, 220, 255)
...
function skin.DrawButton(object)
	...
	-- todo zz
	-- local twidth = font:get_width(object.text)
	-- local theight = font:get_height(object.text)
	local twidth = 50
	local theight = 50	
	...
	if not enabled or not clickable then
		...
		-- button body
		moon.graphics.set_color(255, 255, 255, 255)
		moon.graphics.draw(image, x, y, 0, width, scaley)
		-- button text
		moon.graphics.set_font(font)
		moon.graphics.set_color(textnonclickablecolor)
		moon.graphics.print(text, x + width/2 - twidth/2, y + height/2 - theight/2)
		-- button border
		moon.graphics.set_color(bordercolor)
		skin.OutlinedRectangle(x, y, width, height)
		return
	...
	if down then
		...
		-- button text
		moon.graphics.set_font(font)
		moon.graphics.set_color(textdowncolor)
		moon.graphics.print(text, x + width/2 - twidth/2, y + height/2 - theight/2)
		-- button border
		moon.graphics.set_color(bordercolor)
		skin.OutlinedRectangle(x, y, width, height)
	elseif hover then
		...
		-- button text
		moon.graphics.set_font(font)
		moon.graphics.set_color(texthovercolor)
		moon.graphics.print(text, x + width/2 - twidth/2, y + height/2 - theight/2)
		-- button border
		moon.graphics.set_color(bordercolor)
		skin.OutlinedRectangle(x, y, width, height)
		...
		-- button text
		moon.graphics.set_font(font)
		moon.graphics.set_color(textnohovercolor)
		moon.graphics.print(text, x + width/2 - twidth/2, y + height/2 - theight/2)
		-- button border
		moon.graphics.set_color(bordercolor)
		skin.OutlinedRectangle(x, y, width, height)
	...
	moon.graphics.set_color(255, 255, 255, 150)
...
function skin.DrawCloseButton(object)
	...
	if down then
		-- button body
		moon.graphics.set_color(bodydowncolor)
		moon.graphics.draw(image, x, y)
	elseif hover then
		-- button body
		moon.graphics.set_color(bodyhovercolor)
		moon.graphics.draw(image, x, y)
	else
		-- button body
		moon.graphics.set_color(bodynohovercolor)
		moon.graphics.draw(image, x, y)
	end
...
function skin.DrawImage(object)
	...
	if color then
		...
		moon.graphics.draw(image, x, y, orientation, scalex, scaley, offsetx, offsety, shearx, sheary)
	else
		moon.graphics.set_color(255, 255, 255, 255)
		moon.graphics.draw(image, x, y, orientation, scalex, scaley, offsetx, offsety, shearx, sheary)
	end
...
function skin.DrawImageButton(object)
	...
	if down then
		if image then
			moon.graphics.set_color(255, 255, 255, 255)
			moon.graphics.draw(image, x + 1, y + 1)
		end
		moon.graphics.set_font(font)
		moon.graphics.set_color(0, 0, 0, 255)
		moon.graphics.print(text, x + width/2 - twidth/2 + 1, y + height - theight - 5 + 1)
		moon.graphics.set_color(textdowncolor)
		moon.graphics.print(text, x + width/2 - twidth/2 + 1, y + height - theight - 6 + 1)
	elseif hover then
		if image then
			moon.graphics.set_color(255, 255, 255, 255)
			moon.graphics.draw(image, x, y)
		...
		moon.graphics.set_font(font)
		moon.graphics.set_color(0, 0, 0, 255)
		moon.graphics.print(text, x + width/2 - twidth/2, y + height - theight - 5)
		moon.graphics.set_color(texthovercolor)
		moon.graphics.print(text, x + width/2 - twidth/2, y + height - theight - 6)
	else
		if image then
			moon.graphics.set_color(255, 255, 255, 255)
			moon.graphics.draw(image, x, y)
...
function skin.DrawProgressBar(object)
	...
	-- progress bar body
	moon.graphics.set_color(bodycolor)
	moon.graphics.rectangle("fill", x, y, width, height)
	moon.graphics.set_color(255, 255, 255, 255)
	moon.graphics.draw(image, x, y, 0, barwidth, scaley)
	moon.graphics.set_font(font)
	moon.graphics.set_color(textcolor)
	moon.graphics.print(text, x + width/2 - twidth/2, y + height/2 - theight/2)
...
function skin.DrawTabPanel(object)
	...
	moon.graphics.set_color(bodycolor)
	moon.graphics.rectangle("fill", x, y + buttonheight, width, height - buttonheight)
	moon.graphics.set_color(bordercolor)
...
function skin.DrawTabButton(object)
	...
	if tabnumber == ptabnumber then
		...
		local gradientheight = gradient:get_height()
		local scaley = height/gradientheight
		moon.graphics.set_color(255, 255, 255, 255)
		moon.graphics.draw(gradient, x, y, 0, width, scaley)
		-- button border
		moon.graphics.set_color(bordercolor)
		skin.OutlinedRectangle(x, y, width, height)	
		if image then
			-- button image
			moon.graphics.set_color(255, 255, 255, 255)
			moon.graphics.draw(image, x + 5, y + height/2 - imageheight/2)
			-- button text
			moon.graphics.set_font(font)
			moon.graphics.set_color(texthovercolor)
			moon.graphics.print(text, x + imagewidth + 10, y + height/2 - theight/2)
		else
			-- button text
			moon.graphics.set_font(font)
			moon.graphics.set_color(texthovercolor)
			moon.graphics.print(text, x + 5, y + height/2 - theight/2)
		end	
	else
		-- button body
...
function skin.DrawMultiChoice(object)
	...
	moon.graphics.set_color(bodycolor)
	moon.graphics.rectangle("fill", x + 1, y + 1, width - 2, height - 2)
	...
	if choice == "" then
		...
		moon.graphics.print(choice, x + 5, y + height/2 - theight/2)
	end
	
	moon.graphics.set_color(255, 255, 255, 255)
	moon.graphics.draw(image, x + width - 20, y + 5)
...
function skin.DrawMultiChoiceRow(object)
	...
	if object.hover then
		moon.graphics.set_color(bodyhovecolor)
		moon.graphics.rectangle("fill", x, y, width, height)
		moon.graphics.set_color(texthovercolor)
		moon.graphics.print(text, x + 5, y + 5)
	else
		moon.graphics.set_color(bodynohovercolor)
		moon.graphics.rectangle("fill", x, y, width, height)
		moon.graphics.set_color(textnohovercolor)
...
function skin.DrawTextInput(object)
	...
	if alltextselected then
		...
		if multiline then
			for i=1, #lines do
				...
				-- todo zz
				-- local twidth = font:get_width(str)
				local twidth = 50				
				if twidth == 0 then
					twidth = 5
				end
				moon.graphics.set_color(highlightbarcolor)
				moon.graphics.rectangle("fill", textx, texty + bary, twidth, theight)
				bary = bary + theight
			end
		else
			local twidth = 0
			if masked then
				...
				-- todo zz
				-- twidth = font:get_width(text:gsub(".", maskchar))
				twidth = 50				
			else
				-- todo zz
				-- twidth = font:get_width(text)
				twidth = 50				
			end
			moon.graphics.set_color(highlightbarcolor)
			moon.graphics.rectangle("fill", textx, texty, twidth, theight)
		end
	end
	
	if showindicator and focus then
		moon.graphics.set_color(indicatorcolor)
		moon.graphics.rectangle("fill", indicatorx, indicatory, 1, theight)
	end
	...
	if alltextselected then
		moon.graphics.set_color(textselectedcolor)
	elseif #lines == 1 and lines[1] == "" then
		moon.graphics.set_color(textplaceholdercolor)
	else
		moon.graphics.set_color(textnormalcolor)
	end
	...
	if multiline then
		for i=1, #lines do
			...
			moon.graphics.print(#str > 0 and str or (#lines == 1 and placeholder or ""), textx, texty + theight * i - theight)
		end
	else
		str = lines[1]
		if masked then
		...
		moon.graphics.print(#str > 0 and str or placeholder, textx, texty)
	end
	
	moon.graphics.set_color(230, 230, 230, 255)
	skin.OutlinedRectangle(x + 1, y + 1, width - 2, height - 2)
...
function skin.DrawScrollButton(object)
	...
	if scrolltype == "up" then
		...
		local imagewidth = image:get_width()
		local imageheight = image:get_height()
		-- todo zz
		-- image:setFilter("nearest", "nearest")
		if hover then
		...
		moon.graphics.draw(image, x + width/2 - imagewidth/2, y + height/2 - imageheight/2)
...
function skin.DrawSlider(object)
	...
	if slidtype == "horizontal" then
		moon.graphics.set_color(baroutlinecolor)
		moon.graphics.rectangle("fill", x, y + height/2 - 5, width, 10)
		moon.graphics.set_color(bordercolor)
		moon.graphics.rectangle("fill", x + 5, y + height/2, width - 10, 1)
	elseif slidtype == "vertical" then
		moon.graphics.set_color(baroutlinecolor)
		moon.graphics.rectangle("fill", x + width/2 - 5, y, 10, height)
		moon.graphics.set_color(bordercolor)
		moon.graphics.rectangle("fill", x + width/2, y + 5, 1, height - 10)
	end
...
function skin.DrawCheckBox(object)
	...
	if checked then
		moon.graphics.set_color(checkcolor)
		moon.graphics.rectangle("fill", x + 4, y + 4, width - 8, height - 8)
	end
...
function skin.DrawCollapsibleCategory(object)
	...
	if open then
		...
		-- todo zz
		-- icon:setFilter("nearest", "nearest")
		moon.graphics.draw(icon, x + width - 21, y + 5)
		moon.graphics.set_color(255, 255, 255, 70)
		skin.OutlinedRectangle(x + 1, y + 1, width - 2, 24)
	else
...
function skin.DrawColumnListHeader(object)
	...
	-- todo zz
	-- local twidth = font:get_width(object.name)
	-- local theight = font:get_height(object.name)
	local twidth = 50
	local theight = 50
	...
	if down then
		...
		moon.graphics.set_font(font)
		moon.graphics.set_color(textdowncolor)
		moon.graphics.print(name, x + width/2 - twidth/2, y + height/2 - theight/2)
		-- header border
		moon.graphics.set_color(bordercolor)
...
function skin.DrawColumnListRow(object)
	...
	if selected then
		moon.graphics.set_color(bodyselectedcolor)
		moon.graphics.rectangle("fill", x, y, width, height)
	elseif hover then
		moon.graphics.set_color(bodyhovercolor)
		moon.graphics.rectangle("fill", x, y, width, height)
	elseif colorindex == 1 then
		moon.graphics.set_color(body1color)
		moon.graphics.rectangle("fill", x, y, width, height)
	else
		moon.graphics.set_color(body2color)
		moon.graphics.rectangle("fill", x, y, width, height)
	end
...
function skin.DrawLineNumbersPanel(object)
	...
	-- todo zz
	-- object:SetWidth(10 + font:get_width(#lines))
	object:SetWidth(10 + 50)	
	moon.graphics.set_font(font)
	...
	for i=1, #lines do
		moon.graphics.set_color(textcolor)
		moon.graphics.print(i, x + 5, mody - offsety)
		mody = mody + theight
	end
...
function skin.DrawGrid(object)
	...
	for i=1, object.rows do
		for n=1, object.columns do
			...
			moon.graphics.set_color(bodycolor)
			moon.graphics.rectangle("fill", cx, cy, cw, ch)
			moon.graphics.set_color(bordercolor)
...
function skin.DrawForm(object)
	...
	-- todo zz
	-- local twidth = font:get_width(name)
	local twidth = 50
	
	moon.graphics.set_font(font)
	moon.graphics.set_color(textcolor)
	moon.graphics.print(name, x + 7, y)
	
	moon.graphics.set_color(bordercolor)
	moon.graphics.rectangle("fill", x, y + 7, 5, 1)
	moon.graphics.rectangle("fill", x + twidth + 9, y + 7, width - (twidth + 9), 1)
	moon.graphics.rectangle("fill", x, y + height, width, 1)
	moon.graphics.rectangle("fill", x, y + 7, 1, height - 7)
	moon.graphics.rectangle("fill", x + width - 1, y + 7, 1, height - 7)
...
function skin.DrawMenuOption(object)
	...
	local twidth = smallfont:get_width(text)
	
	
	if option_type == "divider" then
		moon.graphics.set_color(200, 200, 200, 255)
		moon.graphics.rectangle("fill", x + 4, y + 2, width - 8, 1)
		object.contentheight = 10
	else
		moon.graphics.set_font(smallfont)
		if hover then
			moon.graphics.set_color(body_hover_color)
			moon.graphics.rectangle("fill", x + 2, y + 2, width - 4, height - 4)
			moon.graphics.set_color(text_hover_color)
			moon.graphics.print(text, x + 26, y + 5)
		else
			moon.graphics.set_color(text_color)
			moon.graphics.print(text, x + 26, y + 5)
		end
		if icon then
			moon.graphics.set_color(255, 255, 255, 255)
