...
function newobject:update(dt)
	...
	if multiline then
		...
		for k, v in ipairs(lines) do
			local linewidth = 0
			if masked then
				linewidth = font:get_width(v:gsub(".", maskchar))
			else
				linewidth = font:get_width(v)
			end
...
function newobject:draw()
	...
	local stencilfunc = function() moon.graphics.rectangle("fill", x, y, width, height) end
	...
	if vbar and hbar then
		stencilfunc = function() moon.graphics.rectangle("fill", x, y, width - 16, height - 16) end
	end
	
	if loveversion == "0.8.0" then
		local stencil = moon.graphics.newStencil(stencilfunc)
		moon.graphics.setStencil(stencil)
	else
		moon.graphics.setStencil(stencilfunc)
	end
...
function newobject:keypressed(key, unicode)
	...
	if (loveframes.util.IsCtrlDown()) and focus then
		...
		elseif key == "c" and alltextselected and version == "0.9.0" then
			...
			moon.system.setClipboardText(text)
			if oncopy then
				oncopy(self, text)
			end
...
function newobject:RunKey(key, unicode, is_text)
	...
	if key == "left" then
		...
		if not multiline then
			...
			if indicatorx <= x and indicatornum ~= 0 then
				local width = font:get_width(text:sub(indicatornum, indicatornum + 1))
	...
	elseif key == "right" then
		...
		if not multiline then
			...
			if indicatorx >= (x + swidth) and indicatornum ~= #text then
				local width = font:get_width(text:sub(indicatornum, indicatornum))
				self.offsetx = offsetx + width
			elseif indicatornum == #text and offsetx ~= ((font:get_width(text)) - swidth + 10) and font:get_width(text) + textoffsetx > swidth then
				self.offsetx = ((font:get_width(text)) - swidth + 10)
			end
		else
	...
	if key == "backspace" then
		...
		if alltextselected then
			...
			if masked then
				local maskchar = self.maskchar
				cwidth = font:get_width(text:sub(#text):gsub(".", maskchar))
			else
				cwidth = font:get_width(text:sub(#text))
			end
		...
		if unicode > 31 and unicode < 127 then
			...
			if not multiline then
				...
				if masked then
					...
					cwidth = font:get_width(ckey:gsub(".", maskchar))
				else
					twidth = font:get_width(text)
					cwidth = font:get_width(ckey)
				end
...
function newobject:UpdateIndicator()
	...
	for i=1, indicatornum do
		if masked then
			...
			width = width + font:get_width(char)
		else
...
function newobject:Paste()
	...
	local text = moon.system.getClipboardText()
