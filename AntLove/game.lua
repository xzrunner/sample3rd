...
function game.init(difficulty)
	...
	-- game.spraySound = moon.audio.newSource("assets/Shoot.wav" , "static")
	...
	game.blipSound = moon.audio.newSource("assets/Blip.wav", "static")
	map.init()
...
function game.draw()
	...
	for i = 1, 32 do
		...
		if f.code == SOD_FLAG then
			...
			local x = 4 * (i - 1)
			moon.graphics.draw(game.sodImages[f.sodIndex], scale * x, scale * y, 0, scale, scale)
		elseif f.code == PIT_FLAG then
			local y = (f.elevation + ANT_HEIGHT - 2)
			local x = 4 * (i - 1)
			moon.graphics.draw(game.pitImage, scale * x, scale * y, 0, scale, scale)
		end
	end

	for i,sprite in pairs(sprites) do
		moon.graphics.draw(sprite.image, scale * sprite.x, scale * sprite.y, 0, scale, scale)
	end

	for i,raid in pairs(raids) do
		moon.graphics.draw(globalImages.raid, scale * raid.x, scale * raid.y, 0, scale, scale)
	end

	local w = moon.graphics.getWidth()

	if ant.godMode then
		moon.graphics.set_color(0, 0, 0, 128)
		moon.graphics.print("god", w - 100, 0)
	end

	moon.graphics.set_color(0, 0, 0, 255)
	moon.graphics.print(game.lives, w - 20, 0)
end
...
function game.spawn(dt)
	...
	if game.delay >= spawnDelay then
		...
		-- moon.audio.play(game.blipSound)
	end
...
function game.newFloor()
	...
	elseif code == SPRAY_FLAG then
		...
		if game.difficulty > 1 then
			...
			-- moon.audio.play(game.spraySound)
